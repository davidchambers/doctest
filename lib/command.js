      ...
      results.forEach (({path, value}) => {
        ...
        value.forEach (({correct, actual, expected}) => {
          if (!correct) {
            const actualRepr = (
              maybe ('no input')
                    (a => a.throws ? `! ${a.exception}` : show (a.result))
                    (actual)
            );

            const lineRepr = maybe ('') (e => {
              const start = e.lines[0].number;
              const end = e.lines[e.lines.length - 1].number;
              return end === start ? `line ${start}` : `lines ${start}-${end}`;
            });

            const inputCodeRepr = maybe ('') (representLines) (actual);
            const outputCodeRepr = maybe ('') (representLines) (expected);
