...
const Expected = x => ({tag: 'expected', expected: x});
const Actual = x => ({tag: 'actual', actual: x});
const Both = x => y => ({tag: 'both', expected: x, actual: y});
  ...
  queue.reduce ((prev, {input, outputs}) => prev.then (results => (
    new Promise (res => {
      ...
      if (expecteds.length === 0) {
        const {value: {throws, exception}, lines} = (
          actuals.find (({channel}) => channel == null)
        );
        
        if (throws) {
          throw new Error (
            `An exception occurred evaluating ${
      ...
      const testAssertions = actuals.map ((actual, i) => (
        expecteds[i] == null ? Actual (actual) : Both (actual) (expecteds[i])
      ));
      
      const unmetExpectations = (
        (expecteds.slice (actuals.length)).map (Expected)
      );
