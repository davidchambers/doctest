...
(function() {
  var CoffeeScript, R, commonjsEval, doctest, esprima, fetch, fs, functionEval, indent2, indentN, log, noop, pathlib, reduce, repr, rewrite, run, substring, toModule, toPairs, transformComments, wrap;
  ...
  if (typeof window !== 'undefined') {
    CoffeeScript = window.CoffeeScript, esprima = window.esprima, R = window.R;
  ...
  wrap = R.curry(function(type, test) {
    return R.pipe(R.filter(R.partialRight(R.has, test)), R.map(function(dir) {
  ...
  rewrite.js = function(input) {
    ...
    getComments = R.pipe(R.partialRight(esprima.parse, {
      comment: true,
      loc: true
    }), R.prop('comments'));
    ...
    }), R.head, R.partialRight(R.zip, R.append('', R.map(wrap.js, lineTests))), R.flatten, R.join(''))(lineTests);
  ...
  rewrite.coffee = function(input) {
    ...
    }, R.last, R.map(R.converge(R.apply, R.pipe(R.prop('indent'), R.length, indentN), R.pipe(wrap.coffee, R.of))), R.join('\n')), commentChunks);
    ...
    for (_i = 0, _len = queue.length; _i < _len; _i++) {
      ...
      switch (arr.length) {
        ...
        case 2:
          ...
          expected = arr[0]();
          results.push([R.eqDeep(actual, expected), repr(expected), repr(actual), arr[1]]);
          input = noop;
      }
    }
    return results;
