...
const encase = f => (...args) => {
  try {
    return {throws: false, result: f (...args)};
  } catch (exception) {
...
const run = options => queue =>
  queue.reduce ((prev, {input, outputs}) => prev.then (results => (
    new Promise ((res, rej) => {
      ...
      const logFunctions = options.logFunction.map (channel => (
        function log(value) {
          if (open) {
            actuals.push ({channel, value, lines: input.lines});
            clearTimeout (job);
            job = setTimeout (done, options.logTimeout);
          }
        }
      ));
      ...
      return results.concat (actuals.map ((actual, i) => {
        ...
        return {
          correct: (
            ...
            ? exceptionEq (expected.value.exception, actual.value.exception)
            : Z.equals (expected.value.result, actual.value.result)
          ),
          actual: [actual],
          expected: [expected],
        };
      }).concat ((expecteds.slice (outputs.length)).map (expected => ({
        correct: false,
        actual: [],
        expected: [expected],
      }))));
